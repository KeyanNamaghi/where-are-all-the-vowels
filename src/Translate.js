import { validLength, capitalizeFirstLetter } from './utils/utils'
...
const API = (word, setText, setWelshText) => {
  ...
  if (validLength(word)) {
    ...
    //           .then((res) => res.json())
    //           .then((response) => {
    //             console.log("Wasn't in caching server, using google translate API")
    //             setWelshText(capitalizeFirstLetter(response.data.translations[0].translatedText))
    //             setText(capitalizeFirstLetter(word))
    ...
    //               })
    //               .catch((error) => {
    //                 console.log('There was an error caching to the caching server: ', error)
    //               })
    //           })
    //           .catch((error) => {
